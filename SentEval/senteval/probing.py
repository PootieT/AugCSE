...
class PROBINGEval(object):
    ...
    def run(self, params, batcher):
        ...
        for key in self.task_data:
            ...
            sorted_data = sorted(
                zip(self.task_data[key]["X"], self.task_data[key]["y"]), key=lambda z: (len(z[0]), z[1])
            )
        ...
        clf = SplitClassifier(
            X={"train": task_embed["train"]["X"], "valid": task_embed["dev"]["X"], "test": task_embed["test"]["X"]},
            y={"train": task_embed["train"]["y"], "valid": task_embed["dev"]["y"], "test": task_embed["test"]["y"]},
