...
class STSEval(object):
    def loadFile(self, fpath):
        ...
        for dataset in self.datasets:
            sent1, sent2 = zip(
                *[
                    l.split("\t")
                    for l in io.open(fpath + "/STS.input.%s.txt" % dataset, encoding="utf8").read().splitlines()
                ]
            )
            raw_scores = np.array(
                [x for x in io.open(fpath + "/STS.gs.%s.txt" % dataset, encoding="utf8").read().splitlines()]
            )
    ...
    def run(self, params, batcher):
        ...
        results["all"] = {
            "pearson": {"all": all_pearson[0], "mean": avg_pearson, "wmean": wavg_pearson},
            "spearman": {"all": all_spearman[0], "mean": avg_spearman, "wmean": wavg_spearman},
        }
        logging.debug(
            "ALL : Pearson = %.4f, \
            Spearman = %.4f"
            % (all_pearson[0], all_spearman[0])
        )
        logging.debug(
            "ALL (weighted average) : Pearson = %.4f, \
            Spearman = %.4f"
            % (wavg_pearson, wavg_spearman)
        )
        logging.debug(
            "ALL (average) : Pearson = %.4f, \
            Spearman = %.4f\n"
