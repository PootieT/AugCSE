...
class InnerKFoldClassifier(object):
    ...
    def run(self):
        ...
        for train_idx, test_idx in skf.split(self.X, self.y):
            ...
            if self.usepytorch:
                clf = MLP(
                    self.classifier_config, inputdim=self.featdim, nclasses=self.nclasses, l2reg=optreg, seed=self.seed
                )
    ...
    def run(self):
        ...
        for reg in regs:
            ...
            for train_idx, test_idx in skf.split(self.train["X"], self.train["y"]):
                ...
                if self.usepytorch:
                    clf = MLP(
                        self.classifier_config, inputdim=self.featdim, nclasses=self.nclasses, l2reg=reg, seed=self.seed
                    )
