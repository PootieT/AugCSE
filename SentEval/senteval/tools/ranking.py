...
class ImageSentenceRankingPytorch(object):
    ...
    def run(self):
        ...
        trainTxt, trainImg, devTxt, devImg, testTxt, testImg = self.prepare_data(
            self.train["sentfeat"],
            self.train["imgfeat"],
            self.valid["sentfeat"],
            self.valid["imgfeat"],
        ...
        while not stop_train and self.nepoch <= self.maxepoch:
            ...
            logging.info(
                "Dev mean Text to Image: {0}, {1}, {2}, {3}".format(
                    results["t2i"]["r1"], results["t2i"]["r5"], results["t2i"]["r10"], results["t2i"]["medr"]
                )
            )
            logging.info(
                "Dev mean Image to text: {0}, {1}, {2}, {3}".format(
                    results["i2t"]["r1"], results["i2t"]["r5"], results["i2t"]["r10"], results["i2t"]["medr"]
                )
            )
    ...
    def trainepoch(self, trainTxt, trainImg, devTxt, devImg, nepoches=1):
        ...
        for _ in range(self.nepoch, self.nepoch + nepoches):
            ...
            for i in range(0, len(trainTxt), self.batch_size):
                ...
                idximgc = np.random.choice(
                    permutation[:i] + permutation[i + self.batch_size :], self.ncontrast * idx.size(0)
                )
                idxsentc = np.random.choice(
                    permutation[:i] + permutation[i + self.batch_size :], self.ncontrast * idx.size(0)
                )
    ...
    def t2i(self, images, captions):
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(images), self.batch_size):
                img_embed.append(self.model.proj_image(Variable(images[i : i + self.batch_size])))
                sent_embed.append(self.model.proj_sentence(Variable(captions[i : i + self.batch_size])))
    ...
    def i2t(self, images, captions):
        ...
        with torch.no_grad():
            ...
            for index in range(npts):
                ...
                # Compute scores
                scores = torch.mm(query_img.view(1, -1), sent_embed.transpose(0, 1)).view(-1)
