...
def random_deletion(text, prob=0.1, seed=0):
    ...
    text_tf = text_tf if len(text_tf) > 0 else text[np.random.randint(0, N - 1)]
    return [text_tf]


class RandomDeletionOG(SentenceOperation):
    ...
    def generate(self, sentence: str):
        perturbed_texts = random_deletion(text=sentence, prob=self.prob, seed=self.seed)
