...
class TenseTransformation(SentenceOperation):
    ...
    def __init__(self, to_tense: str = "random"):
        super().__init__()
    ...
    def change_tense(self, text, to_tense):
        ...
        for word in doc:
            ...
            if len(words) == 1:
                continue
            if (
                (words[-2].text == "will" and words[-2].tag_ == "MD" and words[-1].tag_ == "VB")
                or words[-1].tag_ in ("VBD", "VBP", "VBZ", "VBN")
                or (not words[-2].text in ("to", "not") and words[-1].tag_ == "VB")
            ):
                ...
                if (words[-2].text == "will" and words[-2].tag_ == "MD") or words[-2].text == "had":
                    out.pop(-1)
                if to_tense == "future":
                    if len(out) > 0 and not (out[-1] == "will" or out[-1] == "be"):
                        out.append("will")
        ...
        # bug where out contains NoneType
        text_out = " ".join([w for w in out if isinstance(w, str)])
    ...
    def generate(self, sentence: str):
        ...
        """
        perturbed_texts = self.change_tense(
            sentence,
            to_tense=random.choice(["past", "present", "future"]) if self.to_tense == "random" else self.to_tense,
        )
