...
class ChangeCityNames:
    ...
    def transform(self, doc, seed=None):
        ...
        sent_words = []
        for i in ents_dict:
            if i["Word"] in list(self.populous_cities) and (i["Entity"] == "GPE" or i["Entity"] == "LOC"):
                sent_words.append("<CITY>")
            else:
                sent_words.append(i["Word"])
        new_sentence = " ".join(sent_words)
        while "<CITY>" in new_sentence:
            rand_city = self.scarce_cities[random.randint(0, len(self.scarce_cities) - 1)]
...
class CityNamesTransformation(SentenceOperation):
    ...
    def __init__(self, seed=0, max_outputs=1, lang="en", data_path=None):
        ...
        if lang == "en":
            ...
            if data_path is None:
                self.transformer = ChangeCityNames(os.path.dirname(os.path.abspath(__file__)), language="es")
            else:
