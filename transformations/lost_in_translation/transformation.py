...
class LostInTranslation(SentenceOperation):
    ...
    def encode_decode(self, text, lang, max_length=600, device=-1):
        ...
        encode = pipeline(
            "translation_en_to_{}".format(lang), model="Helsinki-NLP/opus-mt-en-{}".format(lang), device=device
        )
        decode = pipeline(
            "translation_{}_to_en".format(lang), model="Helsinki-NLP/opus-mt-{}-en".format(lang), device=device
        )
        # en->lang->en
        return decode(encode(text, max_length=max_length)[0]["translation_text"], max_length=max_length)[0][
            "translation_text"
        ]

    def shell(self, text, langs, how, layers, max_length, device, scorer, min_score):
    ...
    def generate(self, sentence: str):
        ...
        return [
            self.shell(
                sentence, self.langs, self.how, self.layers, self.max_length, self.device, self.scorer, self.min_score
            )
        ]
