...
class OperationRuns(object):
    ...
    def _load_single_transformation_test_case(self, transformation_name, search="transformations"):
        ...
        for test_case in load_test_cases(t_js):
            ...
            if filter_instance is None or filter_instance.name() != class_name or prev_class_args != class_args:
    ...
    def _load_all_transformation_test_case(self, heavy=False, search="transformations"):
        if search == "transformations":
            ...
            self._load_multiple_transformation_test_case(
                map_transformation["heavy"] if heavy else map_transformation["light"],
                heavy,
                search,
            )
        elif search == "filters":
    ...
    @staticmethod
    def get_all_folder_names(search="transformations", transformation_name="all") -> Iterable:
    ...
    def get_all_operations(search="transformations") -> Iterable:
        ...
        for (_, folder, _) in iter_modules([transformations_dir]):  # ---> ["back_translation", ...]
            try:
                ...
                print(f"Issue in importing module in folder {folder}: {e}")
                continue
...
def get_implementation(clazz: str, search="transformations"):
    ...
    if operation is not None:
        if operation.name() == clazz:
            return operation
    raise ValueError(f"No class called {clazz} found in the {search} folder. Check if you've spelled it right!")
...
if __name__ == "__main__":
    ...
    for transformation in OperationRuns.get_all_operations_for_task(TaskType.TEXT_CLASSIFICATION):
        print(transformation.name())
        impl = transformation()
        for sentence in [
