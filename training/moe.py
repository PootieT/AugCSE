...
from training.mlp import ProjectionMLP
...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, num_experts, hidden_size, noisy_gating=True, k=4, device="cpu"):
        ...
        self.experts = nn.ModuleList(
            # [MLPLayer(self.hidden_size, num_layers=1, num_heads=1) for i in range(self.num_experts)]
            [ProjectionMLP(self.hidden_size) for i in range(self.num_experts)]
        )
    ...
    def forward(self, x, loss_coef=1e-2, eval=False, **kwargs):
        """Args:
        ...
        """
        gates, load = self.noisy_top_k_gating(x, self.training)  # same as !eval
